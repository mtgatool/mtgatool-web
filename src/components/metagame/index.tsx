...
function Metagame(props: ExportViewProps): JSX.Element {
  ...
  const archMatch = useRouteMatch<{format: string; day: string; arch: string}>(
  ...
  */
  ...
  const getArchetypeDeck = (match: string): void => {
  ...
  return archMatch &&
    ...
    (metagameData.meta.filter((arch) => arch.name == archMatch.params.arch)
      .length === 0 ||
      (deckMatch &&
        metagameData.meta.filter(
          (arch) => arch.name == archMatch.params.arch
        )[0].decks.length <= parseInt(deckMatch.params.deck))) ? (
        ...
        {metagameData && metagameData.meta && archMatch ? (
          ...
          <div className={css["metagame-div"]}>
            {metagameData && metagameData.meta ? (
                metagameData.meta
                .sort(sortArchetypes)
                .map((arch: Archetype, index: number): JSX.Element => {
                  return (
                    <ArchetypeTile
                      id={metagameData._id}
                      key={arch.name + index}
        ...
        {archetype.decks.sort(sortDeckLinks).map((deck, index) => {
          return (
            <Link
              to={(): string =>
                `/metagame/${
                  archMatch.params.format
                }/${
                  archMatch.params.day
                }/${
                  archMatch.params.arch
                }/${
                  index
                }`
              }
